# ZERORE Eval 项目方案说明（PM版）

## 1. 文档目的

本文用于完整说明当前 `ZERORE Eval MVP` 的项目方案，面向产品经理、项目负责人和业务协同方。
...
`ZERORE Eval` 是一个面向长对话数据的评估系统 MVP，用于把原始 chatlog 自动转换为可解释的中间数据，再输出结构化指标、情绪趋势、风险信号、图表和优化建议，帮助团队快速判断 AI 对话质量与可优化方向。
